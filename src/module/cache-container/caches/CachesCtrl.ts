...
export class CachesCtrl {
  ...
  constructor(private $uibModal: IModalService,
    ...
    for (let c of caches) {
      this.cacheService.isAvailable(this.container, c).then((result) => {
        this.cacheAvailability[c.name] = result;
      });
      this.cacheService.isEnabled(this.container.profile, c).then((result) => {
        this.cacheEnablement[c.name] = !result[c.name];
      });
    }
  }
