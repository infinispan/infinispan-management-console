define(["require", "exports", "../../App"], function (require, exports, App_1) {
    "use strict";
    var module = App_1.App.module('managementConsole.api.server-model', []);
    module.factory('ServerModel', [
        function () {
            ...
            Server.prototype.refresh = function () {
                this.getModelController().readAttributeAndResolveExpressions(this.getResourcePath().concat('subsystem', 'datagrid-jgroups'), 'default-stack', true).then(function (response) {
                    this.defaultStack = response.toUpperCase();
                }.bind(this)).catch(function () {
                    this.defaultStack = 'N/A';
                }.bind(this));
                this.getModelController().readAttributeAndResolveExpressions(this.getResourcePath().concat('interface', 'public'), 'inet-address', true).then(function (response) {
            ...
            Server.prototype.fetchCacheStats = function (cache) {
                return this.getModelController()
                    .readResource(this.getResourcePath().concat('subsystem', 'datagrid-infinispan', 'cache-container', cache.cluster.name, cache.type, cache.name), false, true).then(function (response) {
            ...
            Server.prototype.fetchStats = function () {
                return this.getModelController()
                    .readChildrenResources(this.getResourcePath().concat('core-service', 'platform-mbean'), 'type', 2, true, true).then(function (response) {
                    this.stats = response;
                    return response;
                }.bind(this));
            };
            ...
            Server.prototype.fetchAggregateNodeStatsByClusterName = function (clusterName) {
                return this.getModelController()
                    .readResource(this.getResourcePath().concat('subsystem', 'datagrid-infinispan', 'cache-container', clusterName), false, true).then(function (response) {
                    return response;
                }.bind(this));
            };
            Server.prototype.fetchAggregateNodeStats = function () {
                return this.getModelController()
                    .readResource(this.getResourcePath().concat('subsystem', 'datagrid-infinispan', 'cache-container'), true, true).then(function (response) {
                    return response;
                }.bind(this));
            };
...
//# sourceMappingURL=server-model.service.js.map